...
def process_letter_job(letter_id: int) -> None:
    db = SessionLocal()
    try:
        _process_letter_job(db, letter_id)
    except Exception as exc:
        # Catch-all: anything unexpected (a DB error, a bug in this function itself,
        # anything not already handled by the two inner try/excepts below) lands here
        # instead of silently leaving the job stuck at "processing" forever.
        print(f"job for letter {letter_id} failed unexpectedly: {exc!r}")
        db.rollback()  # the failed transaction must be rolled back before we can write again
        job = db.query(Job).filter(Job.letter_id == letter_id).first()
        if job is not None:
            job.status = "failed"
            job.error_message = f"unexpected error: {exc!r}"
            db.commit()
    finally:
        db.close()


def _process_letter_job(db, letter_id: int) -> None:
