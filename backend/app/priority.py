...
def build_letter_priority(extraction: Extraction, today: date) -> Optional[LetterPriority]:
...
def detect_conflicts(
    ...
    for i in range(len(priorities)):
        for j in range(i + 1, len(priorities)):
            ...
            if shared:
                conflicts.append(Conflict(a.letter_id, b.letter_id, "shared_requirement", shared))
            else:
                conflicts.append(
                    Conflict(a.letter_id, b.letter_id, "overlapping_deadlines", f"deadlines {gap} day(s) apart")
                )
